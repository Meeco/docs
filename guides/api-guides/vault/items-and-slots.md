...
```bash
curl --request GET "https://sandbox.meeco.me/item_templates" \
...
```bash
curl --request GET "https://sandbox.meeco.me/item_templates/ITEM-TEMPLATE-ID" \
     -H "Authorization: Bearer $VAULT_ACCESS_TOKEN" \
     -H "Meeco-Organisation-Id: $YOUR_ORGANISATION_ID"
```
...
```bash
curl --request GET "https://sandbox.meeco.me/item_templates?like=SEARCH_TEXT" \
     -H "Authorization: Bearer $VAULT_ACCESS_TOKEN" \
     -H "Meeco-Organisation-Id: $YOUR_ORGANISATION_ID"
```
...
```bash
curl --request PUT "https://sandbox.meeco.me/items/bef961af-aa1f-4f1c-ac95-cdb41b3682db" \
     -H "content-type: application/json" \
     -H "Authorization: Bearer $VAULT_ACCESS_TOKEN" \
...
```bash
curl --request PUT \
 "https://sandbox.meeco.me/items/049740cb-ad1f-43d9-9254-ae25eba30f47" \
...
You receive a shared item by calling `PUT https://sandbox.meeco.me/incoming_shares/{share_id}/accept`. (This indicates you accept the share terms, if any). Next call `GET https://sandbox.meeco.me/incoming_shares/{share_id}/item` and view the Item that has been created in your Vault. Note that `share.item_id` is the <span class="underline">original</span> Item's id, not the one in your Vault!
...
As a result, if `item.share_id` is non-null, then it is a share you received, and you can view the share via `GET https://sandbox.meeco.me/incoming_shares/{item.share_id}`.
