...
The SVX Wallet is a service that provides digital wallets inside an SVX tenant environment. A single instance manages any number of wallets, each of which is a collection of cryptographic keys, [DIDs](../../../platform/did.md) and credentials. The service can be integrated with a mobile or web application, or with a third-party service, via its REST API and client SDKs (JavaScript, Kotlin and Swift).
