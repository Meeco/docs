...
Symmetric keys are stored into a [Keystore](/platform/cryptographic-keys.md) and are encrypted with asymmetric key encryption based upon RSA-4096. The private key for this is derived from a passphrase created by the user and entered during [Vault](/other-products/vault/README.md) creation. In addition to the encryption, the Vault and Keystore are secured through the user’s unique personal log-in and/or device PIN Code.
